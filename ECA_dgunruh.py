...
class ECA(object):
    ...
    def evolve(self,time_steps):
        ...
        --------------
        time_steps: int
            Positive integer specifying the number of time steps to evolve the configuration
        '''
        
        if time_steps < 0:
            raise ValueError("time_steps must be a non-negative integer")
        # try converting time_steps to an int and raise an error if it can't be doen
        try:
    ...
    def create_lookup_table(self, rule_number):
        '''
        Create a ternary rule number and use to create our dictionary, which maps neighborhoods to values
        ...
        ---------------
        lookup_table: dict
            Dictionary mapping tuple neighborhoods to int values (in the range 0-2)
        '''
        # create list of neighborhood tuples in lex. order
        neighborhoods = [(0,0), (0,1), (0,2), (1,0), (1,1), (1,2), (2,0), (2,1), (2,2)]
...
# Let's now make two additional functions. One which will allow us to view what the cellular automata class creates, and another which will create our initial condition
